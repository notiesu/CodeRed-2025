...
/**
 * Convert LaTeX expression to spoken text
...
function validateLatex(latex) {
    ...
    const errors = [
        ...
        /\{[^}]*$/, // Unclosed braces
        /\\[a-zA-Z]+\{[^}]*\{/, // Nested braces without closing
    ];
    
    return !errors.some(error => error.test(latex));
}
...
export function extractMathExpressions(text) {
    const mathPatterns = [
        ...
        /\b\w+\^[0-9]+\b/g, // Superscripts
        /\b\w+_[0-9]+\b/g // Subscripts
    ];
    ...
    mathPatterns.forEach(pattern => {
        ...
        while ((match = pattern.exec(text)) !== null) {
            expressions.push({
                ...
                type: pattern.source.includes('\\') ? 'latex' : 'inline',
