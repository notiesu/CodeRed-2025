...
def image_to_speech():
    ...
    # Read optional voice_id from form data
    voice_id = request.form.get('voice_id') or request.form.get('voiceId') or None
    ...
    # Pass voice preference to the content generator if supported
    gemini_output = generate_content(math_content, voice_id=voice_id)

    #extract speech
    speech_content = gemini_output.get_json().get("response", "")
    # Pass the selected voice to the TTS function
    elevenlabs_output = text_to_speech(speech_content, voice_id=voice_id) if voice_id else text_to_speech(speech_content)
